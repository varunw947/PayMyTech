import { Directive, ElementRef, HostListener, Input } from '@angular/core';
    ...
    export class InputRestrictionDirective {
        ...
        @HostListener('keypress', ['$event']) onKeyPress(event) {
            ...
            }else if (this.appInputRestriction === 'integerWithoutDecimal') {
                this.integerWithoutDecimalOnly(event);
            } else if (this.appInputRestriction === 'noSpecialChars') {
                this.noSpecialChars(event);
            }
            else if (this.appInputRestriction === 'onlyEmailSpecialChar') {
                this.onlyEmailSpecialChar(event);
            }
        }
        ...
        integerWithoutDecimalOnly(event) { 
            const e = <KeyboardEvent>event;
            if (e.key === 'Tab' || e.key === 'TAB') {
                return;
            }
        ...
        onlyEmailSpecialChar(event) {
            ...
            let k;
            k = event.keyCode;  // k = event.charCode;  (Both can be used) 
            if ((k > 64 && k < 91) || (k > 96 && k < 123) || k === 8 || k === 32 || k === 46 || k === 45 || k === 95 || k === 43 || k === 64 || (k >= 48 && k <= 57)) {
                return;
            }
            const ch = String.fromCharCode(e.keyCode);
            const regEx = new RegExp(this.arabicRegex);
