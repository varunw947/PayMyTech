import { Component, OnInit } from '@angular/core';

import {AccountService, AlertService} from '../../services';

import {ReportService} from '../../services/report.service';
import {AdminDistributorService} from '../../services/adminDistributor.service';
import { first } from 'rxjs/operators';
import { Router, ActivatedRoute } from '@angular/router';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';

import {AccordionModule} from 'node_modules/primeng/accordion';
import {MenuItem} from 'primeng/api';
import {AppConfig} from '../../app.config';
import { User } from '../../models';
import {Guid} from '../../GridComponent/gridcomponent.component';


@Component({
  selector: 'app-banner-detail',
  templateUrl: './banner-detail.component.html',
  styleUrls: ['./banner-detail.component.css']
})
export class BannerDetailComponent implements OnInit {

  bannerList:any;
  submitted = false;

  form: FormGroup;

  form2: FormGroup;

   submitted2 = false;
  loading = false;

  model:any={BannerID:0};

  selectedBanner: any ={bannerID:'',bannerDetails:'', bannerURL:'',status:'' };
 
  display: boolean = false;
  displayNew: boolean = false;


  constructor(private accountService: AccountService,private adminDistributorService: AdminDistributorService, private formBuilder: FormBuilder,
    private route: ActivatedRoute,
    private router: Router,private alertService: AlertService , private config:AppConfig){ }

  ngOnInit(): void {
    this.bindBanner();
  }

  bindBanner()
  {
    this.adminDistributorService.getBannerDetails()
    .pipe(first()).subscribe((bannerList:any) => this.bannerList = bannerList);
  }

  editBanner(row:any)
  {
    debugger;

      this.selectedBanner = row;
      this.display = true;
  }

  addBanner()
  {
    debugger;
    
     // this.selectedBanner = row;
      this.displayNew = true;
      this.model.status="Active";
     // this.model=null;
  }

  get f() { return this.form.controls; }

 // get f1() { return this.form2.controls; }

  onSubmit2()
  {
    this.submitted = true;

    // reset alerts on submit
    this.alertService.clear();

    this.loading = true;
    debugger;
   // if(this.model.BannerName!=null)
   // {
         this.adminDistributorService.BannerRegister(this.model)
        .subscribe
        (
          (data:any) => 
            {                
                    alert('Banner Added Successfully !');
                    this.displayNew = false;
                  //  this.model = null;
                  this.model.bannerDetails="";
                    this.model.bannerURL="";
                   
                    this.bindBanner();
              }

           
        )

      // }
      // else
      // {
      //           alert("Enter All Details");
      // }

        this.loading = false;
      
  }


  onSubmit(form:any) {
    this.submitted = true;
    this.alertService.clear();

    this.adminDistributorService.updateBannerDetails(
      this.selectedBanner.bannerID, this.selectedBanner.bannerDetails, this.selectedBanner.bannerURL,this.selectedBanner.status).subscribe((f:any) =>
    {
      
          console.log(f);
        
          this.display = false;
          this.bindBanner();
        
          alert("Data is updated Successfuly");
        
   })
  }

  closePopup()
  {
    this.display = false;
  }

  closePopup2()
  {
    
                    this.model.bannerDetails="";
                    this.model.bannerURL="";
                    
    this.displayNew = false;
   // this.model=null;
  }


}
