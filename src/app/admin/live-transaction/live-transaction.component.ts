...
@Component({
  ...
  styleUrls: ['./live-transaction.component.css']
})
...
search()
{
  this.bindLiveTransactionReportByDate();
...
{ name: 'Retry' }, { name: 'Reprocess'}, { name: 'Reprocessing Pending' }, { name: 'Refund' }, { name: 'Reversal Pending' }
  ...
  constructor(private reportService: ReportService, private formBuilder: FormBuilder,
    ...
    private router: Router) {
     ...
     ngOnInit(): void {
      this.reportName = "LiveTransactionReport_" + this.dateFormat(new Date(),true)+"_"+Guid.newGuid();

     
    }

    bindLiveTransactionReportByDate()
    {
...
// {
       this.reportService.GetLiveRechargeTransactionReportByDate(dateFrom,dateTo,this.selection)
