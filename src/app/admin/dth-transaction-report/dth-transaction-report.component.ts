...
@Component({
  selector: 'app-dth-transaction-report',
  templateUrl: './dth-transaction-report.component.html',
  styleUrls: ['./dth-transaction-report.component.css']
})
export class DthTransactionReportComponent implements OnInit {
...
search()
{
  this.bindDthTransactionReportByDate();
...
public ProviderHeader = [{ provider: 'All' }, { provider: 'DishTV'}, { provider: 'Tata Sky' }, { provider: 'AIRTEL DIGITAL TV' },  { provider: 'RELIANCE BIG TV' }, { provider: 'Videocon D2H' }, { provider: 'SunTV' },];  
  ...
  constructor(private reportService: ReportService, private formBuilder: FormBuilder,
   ...
   getProviders() {  
    ...
    return this.ProviderHeader;  
     ...
     ngOnInit(): void {
      this.reportName = "DthRechargeTransactionReport_" + this.dateFormat(new Date(),true)+"_"+Guid.newGuid();

     
    }

    bindDthTransactionReportByDate()
    {
...
// {
       this.reportService.GetDthRechargeTransactionReportByDate(dateFrom,dateTo,this.selection, this.provider)
